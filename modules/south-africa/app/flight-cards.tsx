    ...
    <div className="relative">
      ...
      <div className="flex gap-4">
        <div className="flex flex-col items-center">
          ...
          <div className="w-0.5 flex-1 bg-blue-500 min-h-[60px]" />
        </div>
        <div className="flex-1 pb-2">
          <div className="flex items-baseline gap-2">
            <span className="font-semibold text-sm">{leg.departureTime}</span>
            <span className="text-sm">{leg.departureLocation} ({leg.departureCode})</span>
          </div>
      ...
      <div className="flex gap-4">
        <div className="flex flex-col items-center">
          <div className={`w-3 h-3 rounded-full ${isLast ? 'bg-blue-500' : 'border-2 border-blue-500 bg-white'}`} />
        </div>
        <div className="flex-1">
          <div className="flex items-baseline gap-2">
            <span className="font-semibold text-sm">{leg.arrivalTime}</span>
            {leg.arrivalDayOffset && (
              <span className="text-xs text-muted-foreground">{leg.arrivalDayOffset}</span>
            )}
            <span className="text-sm">{leg.arrivalLocation} ({leg.arrivalCode})</span>
          </div>
        </div>
      </div>
    </div>
...
function TransferTime({ duration }: { duration: string }) {
  return (
    <div className="flex items-center gap-2 py-3 pl-1 text-xs text-muted-foreground border-l-2 border-dashed border-blue-300 ml-[5px]">
      <span className="ml-4">Transfer time {duration}</span>
    </div>
  )
}
