...
// Hyphens and digits aren't valid in JS identifiers — split on separators,
// PascalCase the segments, then prefix with `_` if the result would start
// with a digit (e.g. module id `2fa-tools` → `_2faTools`).
function moduleIdToIdentifier(id, suffix) {
  const pascal = id
    .split(/[-_]/)
    .filter(Boolean)
 ...
 * Uses static imports (not dynamic loaders) so top-bar icons render in the
 * first paint instead of after a useEffect+chunk-fetch round-trip.
 */
function generateTopBarRegistry(moduleMap) {
  ...
  const decorated = entries.map(e => {
    const identifier = moduleIdToIdentifier(e.id, 'TopBarIcon');
    const prev = seenIdentifiers.get(identifier);
    if (prev) {
      throw new Error(
        `Top-bar icon identifier collision: modules '${prev}' and '${e.id}' both map to '${identifier}'. ` +
        `Rename one module to a non-colliding kebab-case id.`
      );
    }
    seenIdentifiers.set(identifier, e.id);
    return { ...e, identifier };
  });

  const imports = decorated.map(e => `import ${e.identifier} from '${e.importPath}'`).join('\n');
  const mapEntries = decorated.map(e => `  '${e.id}': ${e.identifier},`).join('\n');
 ...
 * To regenerate: pnpm run generate-module-registry
 *
 * Uses static imports so top-bar icons render in the first paint with no
 * chunk-fetch delay. The runtime filter in the consumer still keeps disabled
 * modules' icons out of the DOM.
 */

import type { ComponentType } from 'react'
${imports ? '\n' + imports + '\n' : ''}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const MODULE_TOPBAR_ICONS: Record<string, ComponentType<any>> = {
${mapEntries}
}
`;
}
...
function generateProviderRegistry(moduleMap) {
  ...
  // Pick a per-module local binding so two modules with the same exportName
  // (e.g. both `Provider`) don't collide in this file's import scope.
  const seenBindings = new Map();
  const decorated = entries.map(e => {
    const exportName = e.manifest.globalProvider.exportName || 'default';
    const binding = moduleIdToIdentifier(e.id, 'Provider');
    const prev = seenBindings.get(binding);
    if (prev) {
      throw new Error(
        `Provider binding collision: modules '${prev}' and '${e.id}' both map to '${binding}'. ` +
        `Rename one module to a non-colliding kebab-case id.`
      );
    }
    seenBindings.set(binding, e.id);
    return { ...e, exportName, binding };
  });

  const imports = decorated.map(e => {
    if (e.exportName === 'default') {
      return `import ${e.binding} from '${e.importPath}'`;
    }
    if (e.exportName === e.binding) {
      return `import { ${e.exportName} } from '${e.importPath}'`;
    }
    return `import { ${e.exportName} as ${e.binding} } from '${e.importPath}'`;
  }).join('\n');

  const mapEntries = decorated.map(e => `  '${e.id}': ${e.binding},`).join('\n');
 ...
 *
 * Uses static imports so providers wrap the tree on the first render —
 * no useEffect+chunk-fetch waterfall. Consumer filters by enabled-module
 * id at runtime, so disabled modules' providers never mount.
 */

import type { ComponentType } from 'react'
${imports ? '\n' + imports + '\n' : ''}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const MODULE_PROVIDERS: Record<string, ComponentType<any>> = {
${mapEntries}
}
`;
}
