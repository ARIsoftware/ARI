...
/**
 * Read a module's schema.sql contents, or undefined if it has none.
 * Used to inline SQL into lib/generated/module-schemas.ts so Vercel's
 * function bundles include the SQL — fs.readFile() at runtime would
 * fail with ENOENT on serverless because Next.js doesn't trace files
 * that are only read at runtime.
 */
function readModuleSchemaSql(modulePath) {
  const schemaPath = path.join(modulePath, 'database', 'schema.sql');
  try {
...
/**
 * Generate lib/generated/module-schemas.ts: a typed map from module id to
 * its schema.sql text. The runtime schema-installer reads from this map
 * instead of the filesystem so the install works on Vercel/serverless.
 *
 * Only includes ACTIVE modules (isOverridden: false). An overridden
 * modules-core copy is dead code and must not be installed.
 */
function generateModuleSchemasFile(manifestModules) {
  const entries = [];
  for (const mod of manifestModules) {
    if (mod.isOverridden) continue;
    const sql = readModuleSchemaSql(mod.path);
    if (sql === undefined) continue;
    const escaped = sql
      .replace(/\\/g, '\\\\')
      .replace(/`/g, '\\`')
      .replace(/\$\{/g, '\\${');
    entries.push(`  ${JSON.stringify(mod.id)}: \`${escaped}\`,`);
  }

  const output =
    `// AUTO-GENERATED by scripts/generate-module-registry.js — DO NOT EDIT.\n` +
    `// Source of truth: each module's database/schema.sql\n` +
    `//\n` +
    `// Why this file exists: Vercel/Next.js serverless bundles only include\n` +
    `// files that are statically traced. Reading schema.sql via fs.readFile()\n` +
    `// at runtime ENOENTs on the Lambda because no static import references\n` +
    `// the .sql files. Inlining them here makes the bundle self-contained.\n` +
    `\n` +
  ...
  fs.writeFileSync(MODULE_SCHEMAS_FILE, output, 'utf-8');
}
...
const MODULE_SCHEMAS_FILE = path.join(process.cwd(), 'lib/generated/module-schemas.ts');
 ...
 * Uses next/dynamic loaders so disabled modules' top-bar components are not
 * bundled into the main client chunk. There is a one-frame paint gap on first
 * render for the enabled icons (acceptable for top-bar slots), but disabled
 * modules' code never ships to the client at all — the bundle scales with
 * "modules user has enabled" rather than "modules installed".
 */
function generateTopBarRegistry(moduleMap) {
  ...
  const mapEntries = decorated.map(e =>
    `  '${e.id}': dynamic(() => import('${e.importPath}'), { ssr: false, loading: () => null }),`
  ).join('\n');
 ...
 * To regenerate: pnpm run generate-module-registry
 *
 * Uses next/dynamic so disabled modules' top-bar components are not bundled
 * into the main client chunk. There is a brief paint-time gap on first render
 * (loading: null) — acceptable for top-bar icons. The runtime filter in the
 * consumer still keeps disabled modules' icons out of the DOM.
 */
...
function main() {
  ...
  // Inline each module's schema.sql into a TS map so serverless bundles can
  // install schemas without filesystem reads (the runtime fs.readFile path
  // ENOENTs on Vercel because .sql files aren't statically traced).
  console.log('📝 Generating module schemas TS export...');
  generateModuleSchemasFile(manifest.modules);
  console.log('✅ Module schemas generated at:', MODULE_SCHEMAS_FILE);
