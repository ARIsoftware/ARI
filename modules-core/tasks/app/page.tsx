...
import { TaskRowActions } from '@/modules/tasks/components/task-row-actions'
...
const getTaskAgeColor = (createdAt: string, isStarred: boolean = false) => {
  ...
  return isStarred
    ? 'text-[hsl(var(--primary-foreground))]/75 dark:text-[hsl(var(--muted-foreground))]'
  ...
  return (
    ...
    >
      <span className="relative inline-block">
        ...
        />
        <span
          className={`absolute -bottom-0.5 -right-0.5 inline-block h-1.5 w-1.5 rounded-full ring-1 ring-background ${dot}`}
        />
      </span>
...
export default function TasksPage() {
  ...
  const { data: deletedTasks = [], isLoading: deletedLoading } = useDeletedTasks(
  ...
  const handleKanbanDrop = async (e: React.DragEvent, columnType: string) => {
    ...
    if (columnType === 'pinned') {
      ...
      updates.priority = (columnType.charAt(0).toUpperCase() + columnType.slice(1)) as
        'High' | 'Medium' | 'Low'
    }
  ...
  const handleToggleMask = async (task: Task) => {
    ...
    playTaskSound('tap')
    try {
      await toggleTaskMask(task)
      invalidateTasks() // Sync with server
    } catch (error) {
      console.error('Failed to toggle task privacy:', error)
      toast({
        title: 'Error',
        description: 'Failed to update task privacy. Please try again.',
        variant: 'destructive',
      })
    }
  }
      ...
      <div className="flex flex-1 flex-col gap-6 p-6 min-w-0">
        ...
        {activeFilter === 'Deleted' ? (
                    ...
                    {(task.assigned_agent_id || (task.assignees?.length ?? 0) > 0) && (
                      <div className="flex items-center gap-2 flex-wrap">
                        {task.assigned_agent_id && (
                          <AssignedAgentBadge agentId={task.assigned_agent_id} />
                        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            >
              ...
              <div className="space-y-2">
                {filteredTasks
                  .filter((task) => !task.pinned && task.priority === 'High' && !task.completed)
                  .map((task) => (
                    <div
                      key={task.id}
                      draggable
            ...
            >
              ...
              <div className="space-y-2">
                {filteredTasks
                  .filter((task) => !task.pinned && task.priority === 'Medium' && !task.completed)
                  .map((task) => (
                    <div
                      key={task.id}
                      draggable
            ...
            >
              ...
              <div className="space-y-2">
                {filteredTasks
                  .filter((task) => !task.pinned && task.priority === 'Low' && !task.completed)
                  .map((task) => (
                    <div
                      key={task.id}
                      draggable
            ...
            {filteredTasks.map((task, index) => {
              ...
              return (
                <Fragment key={task.id}>
                  {needsSpacing && viewMode === 'list' && <div className="h-[30px]" />}
                  ...
                  <div
                    ...
                    className={`${
                      ...
                      task.pinned
                        ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] shadow-lg dark:bg-transparent dark:text-[hsl(var(--foreground))] dark:border-[hsl(var(--primary))] dark:shadow-none'
                    ...
                    {viewMode === 'list' ? (
                      <>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-1.5">
                              <span
                                className={`text-sm ${getTaskAgeColor(task.created_at, task.pinned)}`}
                              >
                                {formatTaskAge(task.created_at)}
                              </span>
                            </div>
                        ...
                        <div className="flex flex-wrap items-center justify-end gap-2 mt-1 shrink-0">
                          ...
                          <TaskRowActions
                            ...
                            onEdit={(id) => router.push(`/tasks/edit/${id}`)}
                            onDelete={handleDeleteTask}
                            dragFromHandleRef={dragFromHandleRef}
                          />
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <div className="flex-1">
                          ...
                          <div
                            className={`space-y-2 text-sm ${task.pinned ? 'text-[hsl(var(--primary-foreground))]/75 dark:text-[hsl(var(--muted-foreground))]' : 'text-muted-foreground'}`}
                          >
                            {(task.assigned_agent_id || (task.assignees?.length ?? 0) > 0) && (
                              <div className="flex flex-wrap items-center gap-1.5">
                                {task.assigned_agent_id && (
                            ...
                            <div className="flex items-center gap-1.5">
                              <span
                                className={`text-xs ${getTaskAgeColor(task.created_at, task.pinned)}`}
                              >
                                {formatTaskAge(task.created_at)}
                              </span>
                            </div>
        ...
        <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {taskToDelete?.deleted ? 'Delete Permanently' : 'Move Task to Deleted'}
              </DialogTitle>
              <DialogDescription>
