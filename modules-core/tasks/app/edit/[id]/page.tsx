    ...
    const loadData = async () => {
      ...
      try {
        ...
        const loaded: TaskEditFormValues = {
          ...
          is_private: foundTask.is_private ?? false,
  ...
  const persistTask = async (
    ...
    try {
      const updates = {
        ...
        // Owner-only field: a non-owner's save must not carry is_private at
        // all — the API 403s the whole update if it does.
        ...(canMask ? { is_private: formData.is_private } : {}),
      ...
      <form onSubmit={handleSubmit} className="w-full">
        ...
        <div className="flex gap-3 mt-6">
          ...
          <Button
            type="button"
            variant="outline"
            onClick={() => requestNavigation('/tasks')}
            disabled={loading}
          >
            Cancel
          </Button>
