...
export interface FishData {
  ...
  size: number
  // Speed based on priority score (lower score = higher priority = faster)
  speed: number
  // Vertical position based on priority score (lower score = higher position)
  yPosition: number
  // Color based on due date urgency
  color: string
  // Original task data
  task: Task
  // Priority score (0-1, lower = higher priority)
