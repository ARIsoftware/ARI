...
export function toastError(toast: ToastFn, title: string, error: unknown): void {
  toast({
    variant: 'destructive',
    title,
...
// Folder names go into URLs (eventually) and storage paths, so restrict to
// characters that don't need escaping anywhere: letters, digits, hyphens,
// and underscores.
export const FOLDER_NAME_PATTERN = /^[A-Za-z0-9_-]+$/
export const FOLDER_NAME_MAX_LENGTH = 50

/**
 * Returns a human-readable error message if the name is invalid, or null if
 * it's OK. Used by both the API schemas and the client form so the rules
 * stay in sync.
 */
export function validateFolderName(name: string): string | null {
  ...
  if (!FOLDER_NAME_PATTERN.test(trimmed)) {
    return 'Folder name may only contain letters, numbers, hyphens, and underscores'
  }
  return null
}
