/**
 * Morning Brief - Lightweight LLM client
 *
 * A single non-streaming `callLLM()` that covers all providers in
 ...
 *
 * Mirrors modules-core/module-template/lib/llm-clients.ts so the Morning Brief
...
export interface CallLLMOptions {
  ...
  system: string
  prompt: string
  /** Optional cap on response length. Defaults to 400 — the brief intro is short. */
  maxTokens?: number
}
...
async function callGemini(opts: CallLLMOptions): Promise<{ text: string }> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: { maxOutputTokens: opts.maxTokens ?? 400 },
    }),
  })
  if (!res.ok) {
