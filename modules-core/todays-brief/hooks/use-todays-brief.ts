/**
 * Today's Brief Module - TanStack Query Hooks
 *
 * Data sources:
 * - settings        → module_settings (selected AI provider)
 * - google status   → /api/modules/todays-brief/google/status
 * - greeting        → /api/modules/todays-brief/greeting  (cached per day)
 * - calendar        → /api/modules/todays-brief/calendar  (live, never cached)
...
import { stripSurroundingQuotes } from '@/modules/todays-brief/lib/format'
import type {
  TodaysBriefSettings,
  GoogleStatus,
  IcalStatus,
  ...
  BriefWeather,
  VoicesResponse,
} from '@/modules/todays-brief/types'

const API_BASE = '/api/modules/todays-brief'
...
export const SETTINGS_KEY = ['todays-brief-settings']
export const GOOGLE_STATUS_KEY = ['todays-brief-google-status']
export const ICAL_STATUS_KEY = ['todays-brief-ical-status']
export const GREETING_KEY = ['todays-brief-greeting']
export const CALENDAR_KEY = ['todays-brief-calendar']
export const TOP_TASKS_KEY = ['todays-brief-top-tasks']
export const QUOTE_KEY = ['todays-brief-quote']
export const WEATHER_KEY = ['todays-brief-weather']
export const VOICES_KEY = ['todays-brief-elevenlabs-voices']

// ─── Settings ───────────────────────────────────────────────────────────────
export function useTodaysBriefSettings() {
  return useQuery({
    queryKey: SETTINGS_KEY,
    queryFn: async (): Promise<Partial<TodaysBriefSettings>> => {
...
export function useUpdateTodaysBriefSettings() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async (settings: Partial<TodaysBriefSettings>): Promise<void> => {
      const res = await fetch(`${API_BASE}/settings`, {
        method: 'PUT',
    ...
    onMutate: async (newSettings) => {
      ...
      const previous = queryClient.getQueryData<Partial<TodaysBriefSettings>>(SETTINGS_KEY)
      queryClient.setQueryData<Partial<TodaysBriefSettings>>(SETTINGS_KEY, (old = {}) => ({ ...old, ...newSettings }))
      return { previous }
    },
