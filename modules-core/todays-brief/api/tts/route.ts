/**
 * Today's Brief Module - Read aloud (ElevenLabs text-to-speech)
 *
 * POST /api/modules/todays-brief/tts - Convert the supplied brief text to
 ...
 * - The voice is the user's saved `elevenLabsVoiceId` (Today's Brief settings),
...
import { describeElevenLabsError } from '@/modules/todays-brief/lib/elevenlabs'
import { TtsRequestSchema } from '@/modules/todays-brief/lib/validation'
...
registry.registerPath({
  method: 'post',
  path: '/api/modules/todays-brief/tts',
  operationId: 'readTodaysBriefAloud',
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!upstream.ok || !upstream.body) {
      ...
      console.error('todays-brief tts upstream error:', upstream.status, raw.slice(0, 300))
    ...
    console.error('POST /api/modules/todays-brief/tts error:', error instanceof Error ? error.message : error)
