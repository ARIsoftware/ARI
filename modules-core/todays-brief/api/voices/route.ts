/**
 * Today's Brief Module - ElevenLabs voices API
 *
 * GET /api/modules/todays-brief/voices - List the ElevenLabs voices available
...
import { VoicesResponseSchema } from '@/modules/todays-brief/lib/validation'
import { registry } from '@/lib/openapi/registry'
import { DEFAULT_SECURITY, ErrorResponseSchema, InternalServerErrorResponse, UnauthorizedResponse } from '@/lib/openapi/common'

registry.registerPath({
  method: 'get',
  path: '/api/modules/todays-brief/voices',
  operationId: 'listTodaysBriefVoices',
...
export async function GET() {
  try {
    ...
    if (!upstream.ok) {
      ...
      console.error('todays-brief voices upstream error:', upstream.status, raw.slice(0, 300))
    ...
    console.error('GET /api/modules/todays-brief/voices error:', error instanceof Error ? error.message : error)
