/**
 * Today's Brief Module - Calendar API
 *
 * GET /api/modules/todays-brief/calendar
 *
...
import { CalendarResponseSchema } from '@/modules/todays-brief/lib/validation'
import { getGoogleConfig, refreshAccessToken, fetchTodaysEvents } from '@/modules/todays-brief/lib/google'
import { fetchTodaysIcalEvents } from '@/modules/todays-brief/lib/ical'
import { registry } from '@/lib/openapi/registry'
import { DEFAULT_SECURITY, ErrorResponseSchema, InternalServerErrorResponse, UnauthorizedResponse } from '@/lib/openapi/common'
import { todaysBriefGoogleTokens, todaysBriefIcalSubscriptions, userPreferences } from '@/lib/db/schema'
...
registry.registerPath({
  method: 'get',
  path: '/api/modules/todays-brief/calendar',
  operationId: 'getTodaysBriefCalendar',
...
export async function GET() {
  try {
    ...
    const [prefsRows, icalRows, tokenRows] = await Promise.all([
      ...
      withRLS((db) =>
        db.select({ icsUrl: todaysBriefIcalSubscriptions.icsUrl })
          .from(todaysBriefIcalSubscriptions)
    ...
    if (icalRow?.icsUrl) {
      try {
        ...
        console.error('todays-brief ical fetch error:', err instanceof Error ? err.message : err)
    ...
    if (!accessToken) {
      ...
      try {
        ...
        await withRLS((db) =>
          db.update(todaysBriefGoogleTokens)
            .set({
              accessToken: encrypt(accessToken as string),
              tokenExpiresAt: expiresAt,
              updatedAt: new Date().toISOString(),
            })
            .where(and(eq(todaysBriefGoogleTokens.id, tokenRow.id), eq(todaysBriefGoogleTokens.userId, user.id)))
        )
      } catch (err) {
        ...
        console.error('todays-brief calendar refresh error:', msg)
    ...
    try {
      ...
      console.error('todays-brief calendar fetch error:', err instanceof Error ? err.message : err)
      return createErrorResponse('Could not load your Google Calendar. Please try again.', 502)
    }
  } catch (error) {
    console.error('GET /api/modules/todays-brief/calendar error:', error instanceof Error ? error.message : error)
