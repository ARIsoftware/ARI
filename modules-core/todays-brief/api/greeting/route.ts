 ...
 * todays_brief_greetings; every later visit that day reuses it. The greeting
...
import { greetingQuerySchema, GreetingResponseSchema } from '@/modules/todays-brief/lib/validation'
import { getProviderCredentials } from '@/modules/todays-brief/lib/provider-keys'
import { callLLM } from '@/modules/todays-brief/lib/llm-clients'
import { getLocalDateString } from '@/modules/todays-brief/lib/google'
import { AI_PROVIDERS, type AiProviderId } from '@/lib/ai-providers'
import { registry } from '@/lib/openapi/registry'
import { DEFAULT_SECURITY, ErrorResponseSchema, InternalServerErrorResponse, UnauthorizedResponse } from '@/lib/openapi/common'
import { moduleSettings, userPreferences, todaysBriefGreetings } from '@/lib/db/schema'
import { and, eq } from 'drizzle-orm'

export const dynamic = 'force-dynamic'

const MODULE_ID = 'todays-brief'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const loadCached = () =>
      withRLS((db) =>
        db.select({ message: todaysBriefGreetings.message, provider: todaysBriefGreetings.provider, model: todaysBriefGreetings.model })
          .from(todaysBriefGreetings)
          .where(and(eq(todaysBriefGreetings.userId, user.id), eq(todaysBriefGreetings.briefDate, briefDate)))
          .limit(1)
      )
    ...
    if (!provider) {
      return createErrorResponse("No AI provider selected. Choose one in Today's Brief settings.", 400)
    }
    ...
    try {
      const result = await callLLM({
        ...
        maxTokens: 200,
        system:
          'You are a warm, articulate executive secretary writing the opening lines of your principal\'s daily brief. ' +
      ...
      console.error('todays-brief greeting upstream error:', err instanceof Error ? err.message : err)
    ...
    console.error('GET /api/modules/todays-brief/greeting error:', error instanceof Error ? error.message : error)
