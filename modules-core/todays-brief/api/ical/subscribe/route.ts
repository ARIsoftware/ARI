/**
 * Today's Brief Module - Subscribe to a calendar feed
 *
 * POST /api/modules/todays-brief/ical/subscribe  { url }
 *
...
import { IcalSubscribeSchema, IcalStatusResponseSchema } from '@/modules/todays-brief/lib/validation'
import { fetchIcsText, looksLikeIcs } from '@/modules/todays-brief/lib/ical'
...
registry.registerPath({
  method: 'post',
  path: '/api/modules/todays-brief/ical/subscribe',
  operationId: 'subscribeTodaysBriefIcal',
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('todays-brief ical subscribe validation error:', msg)
    ...
    await withRLS((db) =>
      db.insert(todaysBriefIcalSubscriptions)
        .values({ userId: user.id, icsUrl: encrypted })
        .onConflictDoUpdate({
          target: todaysBriefIcalSubscriptions.userId,
    ...
    console.error('POST /api/modules/todays-brief/ical/subscribe error:', error instanceof Error ? error.message : error)
