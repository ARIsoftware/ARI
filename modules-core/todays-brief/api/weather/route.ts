/**
 * Today's Brief Module - Weather API
 *
 * GET /api/modules/todays-brief/weather
 *
...
import { WeatherResponseSchema } from '@/modules/todays-brief/lib/validation'
import {
  geocodeCity,
  fetchDailyWeather,
  weatherDescription,
  getTemperatureUnit,
  type TempUnit,
} from '@/modules/todays-brief/lib/weather'
...
registry.registerPath({
  method: 'get',
  path: '/api/modules/todays-brief/weather',
  operationId: 'getTodaysBriefWeather',
...
export async function GET() {
  try {
    ...
    try {
      ...
      console.error('todays-brief weather lookup error:', err instanceof Error ? err.message : err)
      return unavailable('C')
    }
  } catch (error) {
    console.error('GET /api/modules/todays-brief/weather error:', error instanceof Error ? error.message : error)
