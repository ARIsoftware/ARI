...
import type { StorageProviderInterface, StorageProvider, BackupManagerSettings } from '../../types'
...
export function getStorageProvider(
  ...
  switch (provider) {
    case 'supabase':
      return new SupabaseStorageProvider(settings.supabase?.bucketName || 'ari-backups')
    case 'r2':
...
export function isProviderConfigured(provider: StorageProvider): { configured: boolean; missing: string[] } {
  ...
  switch (provider) {
    case 'supabase':
      // Supabase is always configured via existing credentials
