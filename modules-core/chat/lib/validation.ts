...
// Shared cap for model-id fields — the same constant AiProviderCard uses for
// its input maxLength, so the client affordance and server validation agree.
const modelIdMax = (schema: z.ZodString) =>
  schema.max(MODEL_ID_MAX_LENGTH, `Model must be ${MODEL_ID_MAX_LENGTH} characters or fewer`)
...
export const createConversationSchema = z
  .object({
    ...
    provider: z.enum(PROVIDERS, {
      errorMap: () => ({
        message: 'Provider must be one of: openai, anthropic, gemini, openrouter',
      }),
    }),
    model: modelIdMax(z.string().min(1, 'Model is required')),
  })
...
export const sendMessageSchema = z
  .object({
    content: z
      ...
      .max(50000, 'Message must be 50000 characters or fewer'),
    attachments: z
      .array(SendAttachmentSchema)
      .max(10, 'Cannot attach more than 10 files in a single message')
      .optional(),
  })
...
export const chatListQuerySchema = z
  .object({
    limit: z.coerce
      ...
      .max(1000)
      .catch(1000)
      .openapi({ type: 'integer', default: 1000 }),
...
export const ChatConversationSingleResponseSchema = z
  .object({
    conversation: ChatConversationSchema,
  })
  .openapi('ChatConversationSingleResponse')

export const ChatConversationDetailResponseSchema = z
  .object({
    conversation: ChatConversationSchema,
...
export const ChatUploadListResponseSchema = z
  .object({
...
export const ChatUploadSingleResponseSchema = z
  .object({
...
export const ChatSettingsSchema = z
  .object({
    ...
    defaultModel: modelIdMax(z.string()).optional(),
  })
  .strict()
