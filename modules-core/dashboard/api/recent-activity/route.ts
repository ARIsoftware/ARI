...
export async function GET(_request: NextRequest) {
  ...
  const [recentTasks, recentContacts] = await withRLS(async (db) => {
    const t = await db
      ...
      .from(tasks)
      // Per-record privacy: tasks masked by another user are invisible here.
      // (Inline predicate — modules must not import sibling module internals.)
      .where(sql`(${tasks.isPrivate} IS NOT TRUE OR ${tasks.userId} = ${user.id})`)
