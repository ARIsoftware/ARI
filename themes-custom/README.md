...
After adding or editing a theme, restart ARI (`./ari start`) or run `pnpm generate-module-registry` — themes are compiled into the app at startup, not read live. Your theme then appears in Settings → Themes automatically.

Notes:

- A broken custom theme is skipped with a warning at startup; it never breaks ARI.
