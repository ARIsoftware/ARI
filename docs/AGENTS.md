...
- Start: `./ari start` (starts Supabase + dev server, requires Docker).
- Stop: `./ari stop` (stops Supabase containers).
- Status: `./ari status` (shows Supabase status).
- Install: `pnpm install` (pnpm is preferred; lockfile present).
- Develop (manual): `pnpm dev` (runs Next.js locally, assumes Supabase already running).
...
- Start (production): `pnpm start` (serve the build).
- Node: use Node 18+.
...
## Environment & Secrets
- Required: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (use new publishable key format `sb_publishable_...`; legacy `NEXT_PUBLIC_SUPABASE_ANON_KEY` is still honored as a fallback), and `OPENAI_API_KEY` (chat route); server-only `SUPABASE_SECRET_KEY` powers backup exports/imports.
...
For local development, `.env.supabase.local` is auto-generated by the installer and `./ari start`. You do not need to create environment files manually. The file includes:
...
BETTER_AUTH_SECRET=<auto-generated, preserved across restarts>
BETTER_AUTH_URL=http://localhost:3000
ARI_FIRST_RUN_ADMIN_EMAIL=local@ari.software
ARI_FIRST_RUN_ADMIN_PASSWORD=<auto-generated>
```

### Production / Cloud Supabase
For production, configure `.env.local`:
