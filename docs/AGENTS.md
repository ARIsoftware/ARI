...
### Local Development
`.env.supabase.local` is auto-generated by the installer and `./ari start`. It contains only the local Supabase connection vars:
...
All other env vars (`BETTER_AUTH_SECRET`, `ARI_FIRST_RUN_ADMIN_*`, etc.) are configured in `.env.local` via the `/welcome` setup flow.
