...
// When `localSupabaseDetected` is true and no explicit databaseUrl is set,
// the Supabase block collapses to a pointer at the auto-generated
// .env.supabase.local (since a local Supabase is already wired up there).
export function renderEnvFile(
  fields: WelcomeEnvFields,
  options: { localSupabaseDetected?: boolean } = {},
): string {
  const lines: string[] = []
  ...
  if (options.localSupabaseDetected && !dbUrl) {
    lines.push(
