...
export async function testFitnessDatabase(userId?: string) {
  ...
  try {
    ...
    // Test 3: Try to insert a test record (only if userId provided)
    if (userId) {
      ...
      console.log("Skipping insert test - no userId provided")
    }
