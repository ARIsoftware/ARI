...
} from '@/modules-core/todays-brief/lib/validation'
import { AI_PROVIDER_IDS } from '@/lib/ai-providers'

// ─── TodaysBriefSettingsSchema ───────────────────────────────────────────────

describe('TodaysBriefSettingsSchema', () => {
  it('accepts empty object (all optional)', () => {
    expect(TodaysBriefSettingsSchema.safeParse({}).success).toBe(true)
  })

  it('accepts null selectedAiProvider', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ selectedAiProvider: null }).success).toBe(true)
  })

  it('accepts valid AI provider', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ selectedAiProvider: 'claude' }).success).toBe(true)
  })

  it('rejects invalid AI provider for selectedAiProvider', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ selectedAiProvider: 'unknown-ai' }).success).toBe(false)
  })

  it('accepts all AI_PROVIDER_IDS for selectedAiProvider', () => {
    for (const id of AI_PROVIDER_IDS) {
      expect(TodaysBriefSettingsSchema.safeParse({ selectedAiProvider: id }).success).toBe(true)
    }
  })

  it('accepts null selectedVoiceProvider', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ selectedVoiceProvider: null }).success).toBe(true)
  })

  it('accepts valid voice provider', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ selectedVoiceProvider: 'elevenlabs' }).success).toBe(true)
  })

  it('rejects invalid voice provider', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ selectedVoiceProvider: 'unknown' }).success).toBe(false)
  })

  it('accepts null elevenLabsVoiceId', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ elevenLabsVoiceId: null }).success).toBe(true)
  })

  it('accepts valid elevenLabsVoiceId within 100 chars', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ elevenLabsVoiceId: 'voice-abc-123' }).success).toBe(true)
  })

  it('rejects elevenLabsVoiceId exceeding 100 chars', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ elevenLabsVoiceId: 'a'.repeat(101) }).success).toBe(false)
  })

  it('accepts aiProviderModels map', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ aiProviderModels: { claude: 'claude-3-opus' } }).success).toBe(true)
  })

  it('rejects provider id exceeding 64 chars in aiProviderModels', () => {
    const longKey = 'a'.repeat(65)
    expect(TodaysBriefSettingsSchema.safeParse({ aiProviderModels: { [longKey]: 'model' } }).success).toBe(false)
  })

  it('rejects model id exceeding MODEL_ID_MAX_LENGTH (200)', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ aiProviderModels: { openai: 'x'.repeat(201) } }).success).toBe(false)
  })

  it('rejects unknown extra keys (strict)', () => {
    expect(TodaysBriefSettingsSchema.safeParse({ unknownField: true }).success).toBe(false)
  })
})
