/**
 * Tests for todays-brief/lib/google.ts
 *
 * Mocks: global fetch. Tests all exported functions.
 */
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest'

// BriefMeeting type comes from modules-core/todays-brief/types — mock it
vi.mock('@/modules/todays-brief/types', () => ({
...
import {
  ...
  GOOGLE_SCOPE,
  OAUTH_STATE_COOKIE,
} from '@/modules-core/todays-brief/lib/google'
...
describe('getRedirectUri', () => {
  ...
  it('derives from BETTER_AUTH_URL when no explicit override', () => {
    ...
    expect(uri).toBe('https://myapp.example.com/api/modules/todays-brief/google/callback')
  })
  ...
  it('derives from NEXT_PUBLIC_APP_URL as second fallback', () => {
    ...
    const uri = getRedirectUri()
    expect(uri).toBe('https://fallback.example.com/api/modules/todays-brief/google/callback')
  })
  ...
  it('defaults to localhost:3000 when no env vars set', () => {
    ...
    expect(uri).toContain('/api/modules/todays-brief/google/callback')
  })
