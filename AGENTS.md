...
For local development, `.env.supabase.local` is auto-generated by the installer and `./ari start`. It includes all required variables and overrides `.env.local` via `next.config.mjs` (dotenv with `override: true`). You do not need to create this file manually.

```env
# Auto-generated by ./ari start
...
2. For local dev, use the auto-generated admin email/password shown after install
